...
(function( $ ) {
    ...
    var defaults = {
        ...
        exports: '> thead > tr > th > form.filters:first > div.export'
    };
    ...
    function LoadList( index ) {
        ...
        opts.exports = $(opts.table).find(opts.exports);
        ...
        opts.exports.find('button').click(function() {
            var div = $(this).parents('div:first');
            if (!div.length) {
                alert('Кнопка экспорта должна быть в div');
                return;
            }
            var url = div.find('input.url:first');
            if (!url.length) {
                alert('Рядом с кнопкой экспорта должен быть hidden.url');
                return;
            }
            url = url.val();
            var burl = $(opts.filters).attr('action');
            var bmethod = $(opts.filters).attr('method');
            $(opts.filters).attr('action', url);
            $(opts.filters).attr('method', 'post');
            $(opts.filters).submit();

            $(opts.filters).attr('action', burl);
            $(opts.filters).attr('method', bmethod);
        });
    ...
    function UpdateList( index ) {
        ...
        opts.ajax = $.ajax({
            ...
            complete: function( jqXHR, textStatus ){
                ...
                // Нулевой статус ошибки детектим как отмену
    ...
    function _column_count( obj ) {
        var count = 0;
        $(obj).find('tr:first').first().find('>td, >th').each(function () {
